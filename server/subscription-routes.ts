...
export function registerSubscriptionRoutes(app: Express, storage: any) {
  ...
  // Check subscription limits before creating quiz
  app.use("/api/quiz", async (req: Request, res: Response, next: any) => {
    // Only check limits on POST (quiz creation)
    if (req.method !== 'POST') {
      return next();
    }
    ...
    try {
      ...
      const plan = SUBSCRIPTION_PLANS[userData.subscriptionPlan as keyof typeof SUBSCRIPTION_PLANS] || SUBSCRIPTION_PLANS.free;
      const quizLimit = plan.limits.quizzesPerDay;

      // Check if user has reached their daily limit
      if (quizLimit > 0 && userData.dailyQuizCount >= quizLimit) {
        return res.status(403).json({ 
          ...
          currentCount: userData.dailyQuizCount,
          limit: quizLimit,
        });
      }
      ...
      await storage.updateUser(user.id, {
        dailyQuizCount: (userData.dailyQuizCount || 0) + 1,
      });

      next();
    } catch (error) {
      console.error("Error checking subscription limits:", error);
      next(); // Continue even if limit check fails
    }
  });
}